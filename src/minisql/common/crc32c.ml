...
// Immutable reflected Castagnoli table. Precomputing one complete byte replaces
// eight data-dependent polynomial iterations per input byte while preserving
// the exact CRC-32C state transition used by every existing on-disk checksum.
crcTable = [
...
function update(previous, buffer, offset, length)
  ...
  index = offset
  limit = offset + length
  // Eight-way unrolling removes seven loop branches from the hot path. Every
  // transition is still a normal reflected CRC byte update, so incremental
  // calls and arbitrary offsets remain bit-for-bit compatible.
  while index <= limit - 8
    crc = crcTable[(crc ^ buffer[index]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 1]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 2]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 3]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 4]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 5]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 6]) & 255] ^ (crc >> 8)
    crc = crcTable[(crc ^ buffer[index + 7]) & 255] ^ (crc >> 8)
    index = index + 8
  end while
  while index < limit
    crc = crcTable[(crc ^ buffer[index]) & 255] ^ (crc >> 8)
