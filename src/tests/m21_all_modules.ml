...
function check(actual, expected, label)
  if actual != expected then
    print "M21 module smoke mismatch: " + label + " expected=" + expected + " actual=" + actual
    return 1
  end if
  return 0
end function
...
function checkBool(actual, label)
  if not actual then
    print "M21 module smoke mismatch: " + label
    return 1
  end if
  return 0
end function
...
function main(args)
  ...
  if failures != 0 then
    print "MiniSQL M21 module smoke test: FAIL (failures=" + failures + ")"
    return 1
  end if
  print "MiniSQL M21 module smoke test: SUCCESS (71 modules)"
  return 0
end function
