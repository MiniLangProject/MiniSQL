...
function main(args)
  ...
  test.record(state, buffer_pool.cachedRowCount(readCache, args[0]) is void, "row-count cache starts empty")
  test.equal(state, buffer_pool.rememberRowCount(readCache, args[0], 7), 7, "row-count cache publication")
  test.equal(state, buffer_pool.cachedRowCount(readCache, args[0]), 7, "row-count cache hit")
  ...
  test.record(state, buffer_pool.cachedRowCount(readCache, args[0]) is void, "row-count cache invalidation")
