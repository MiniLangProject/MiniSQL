...
function main(args)
  ...
  executeOne(engine, "CREATE INDEX idx_optimizer_correlated_first ON optimizer_correlated(first_key)")
  executeOne(engine, "CREATE INDEX idx_optimizer_correlated_second ON optimizer_correlated(second_key)")
  executeOne(engine, "ANALYZE optimizer_correlated")
  intersectionPlan = executeOne(engine, "EXPLAIN SELECT id FROM optimizer_correlated WHERE first_key = 3 AND second_key = 3")
  testkit.record(state, planContains(intersectionPlan, "Index Intersection"), "optimizer selects row-identity index intersection")
  intersectionRows = executeOne(engine, "SELECT id FROM optimizer_correlated WHERE first_key = 3 AND second_key = 3")
  testkit.equal(state, len(intersectionRows.rows), 20, "index intersection preserves correlated matches")
  ...
  unionPlan = executeOne(engine, "EXPLAIN SELECT id FROM optimizer_fact WHERE id = 1 OR category = 3")
  testkit.record(state, planContains(unionPlan, "Index Union"), "optimizer selects complete OR index union")
  unionRows = executeOne(engine, "SELECT id FROM optimizer_fact WHERE id = 1 OR category = 3")
  testkit.equal(state, len(unionRows.rows), 31, "index union deduplicates and preserves OR matches")
  ...
  executeOne(engine, "CREATE INDEX idx_optimizer_large_join_key ON optimizer_large(join_key)")
  inferredPlan = executeOne(engine, "EXPLAIN SELECT s.id, l.id FROM optimizer_small s INNER JOIN optimizer_large l ON s.join_key = l.join_key WHERE s.join_key = 3")
  testkit.record(state, planContains(inferredPlan, "Index Scan [optimizer_large index=idx_optimizer_large_join_key"), "join equality propagates a constant to the indexed peer")
  testkit.record(state, planContains(inferredPlan, "Index Nested Loop Join"), "transitive peer index retains the parameterized join path")
  inferredRows = executeOne(engine, "SELECT s.id, l.id FROM optimizer_small s INNER JOIN optimizer_large l ON s.join_key = l.join_key WHERE s.join_key = 3")
  testkit.equal(state, len(inferredRows.rows), 40, "transitive join predicate preserves result rows")
