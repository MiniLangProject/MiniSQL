...
function checkComponent(actual, expected)
  if actual != expected then
    print "M0 module identity mismatch: expected=" + expected + " actual=" + actual
    return 1
  end if
  return 0
end function
...
function main(args)
  ...
  if failures != 0 then
    print "MiniSQL M0 module smoke test: FAIL (failures=" + failures + ")"
    return 1
  end if

  print "MiniSQL M0 module smoke test: SUCCESS (71 modules)"
  return 0
end function
