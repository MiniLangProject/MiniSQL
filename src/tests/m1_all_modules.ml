...
function checkComponent(actual, expected)
  if actual != expected then
    print "M1 module identity mismatch: expected=" + expected + " actual=" + actual
    return 1
  end if
  return 0
end function
...
function main(args)
  ...
  if m_common_endian.targetMilestone() != "M1" then
    print "M1 endian target milestone mismatch"
    failures = failures + 1
  end if
  if not m_common_endian.isImplemented() then
    print "M1 endian implementation marker is false"
    failures = failures + 1
  end if
  ...
  if failures != 0 then
    print "MiniSQL M1R1 module smoke test: FAIL (failures=" + failures + ")"
    return 1
  end if

  print "MiniSQL M1R1 module smoke test: SUCCESS (71 modules)"
  return 0
end function
