...
function main(args)
  ...
  testkit.record(state, len(alice.salt) == uuid.PASSWORD_SALT_BYTES and len(alice.verifier) == uuid.PASSWORD_CREDENTIAL_BYTES, "new password material uses the hardened credential length")
  testkit.record(state, alice.iterations == uuid.DEFAULT_PBKDF2_ITERATIONS, "PBKDF2 work factor")
  legacySalt = bytes(uuid.PASSWORD_SALT_BYTES, 7)
  legacyVerifier = uuid.deriveKey(bytes("Legacy-Password-21!"), legacySalt, uuid.DEFAULT_PBKDF2_ITERATIONS, uuid.PASSWORD_VERIFIER_BYTES)
  legacyPrincipal = metadata.createPrincipal(102, "legacy_login", metadata.PRINCIPAL_USER, true, true, false, false, legacySalt, uuid.DEFAULT_PBKDF2_ITERATIONS, legacyVerifier)
  testkit.record(state, metadata.isPrincipalMetadata(legacyPrincipal) and uuid.authenticationScheme(legacyPrincipal.verifier) == uuid.AUTH_SCHEME_LEGACY, "legacy 32-byte catalog credentials remain readable")
  testkit.record(state, uuid.verifyPassword("Legacy-Password-21!", legacyPrincipal.salt, legacyPrincipal.iterations, legacyPrincipal.verifier), "legacy password credentials remain verifiable")
