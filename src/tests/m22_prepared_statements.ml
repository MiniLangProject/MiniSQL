...
function main(args)
  ...
  firstHandleStats = database_manager.readHandleStats(managed)
  testkit.equal(state, firstHandleStats.indexHandles, 1, "first indexed SELECT caches one index handle")
  testkit.equal(state, firstHandleStats.tableHandles, 1, "first indexed SELECT caches one table handle")
  testkit.equal(state, firstHandleStats.misses, 2, "first indexed SELECT opens two storage handles")
  repeatedFound = executeOne(engine, "EXECUTE find_item USING 2")
  repeatedHandleStats = database_manager.readHandleStats(managed)
  testkit.equal(state, repeatedFound.rows[0][0].value, "beta", "repeated prepared SELECT result")
  testkit.equal(state, repeatedHandleStats.hits, 2, "repeated indexed SELECT reuses both storage handles")
  testkit.equal(state, repeatedHandleStats.misses, 2, "repeated indexed SELECT opens no storage handles")
  ...
  invalidatedHandleStats = database_manager.readHandleStats(managed)
  testkit.equal(state, invalidatedHandleStats.indexHandles, 0, "write invalidates cached index handles")
  testkit.equal(state, invalidatedHandleStats.tableHandles, 0, "write invalidates cached table handles")
