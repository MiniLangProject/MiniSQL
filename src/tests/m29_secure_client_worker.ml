...
function main(args)
  ...
  if typeof(active) == "error" then print "ERROR open " + active.code + ": " + active.message; return 1 end if
  ...
  if typeof(selected) == "error" then ignored = try(client.close(active)); print "ERROR query " + selected.code + ": " + selected.message; return 1 end if
  ...
  if typeof(pong) == "error" then print "ERROR ping " + pong.code + ": " + pong.message; return 1 end if
  if typeof(closed) == "error" then print "ERROR close " + closed.code + ": " + closed.message; return 1 end if
