...
// Runs the secret handling test scenario. It returns zero only after all required invariants pass; invalid arguments, setup failures, or failed assertions produce a non-zero status.
function main(args)
  ...
  testkit.record(state, len(material.verifier) == uuid.PASSWORD_CREDENTIAL_BYTES, "SCRAM credential length")
  ...
  nonce = bytes(uuid.AUTH_NONCE_BYTES, 7)
  stolenLegacyProof = uuid.authProof(slice(material.verifier, 0, uuid.PASSWORD_VERIFIER_BYTES), nonce, "stolen", "client")
  testkit.record(state, not uuid.verifyScramClientProof(material.verifier, nonce, "stolen", stolenLegacyProof), "stored credential cannot be replayed as a password verifier")
  uuid.wipeSecret(stolenLegacyProof)
