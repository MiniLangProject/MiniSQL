...
function runConfiguredServer(mode, databasePath, configPath)
  ...
  if mode == "tls" then
    ...
    return serverResult(try(server.serveTlsAddressWithOperationalLimits(databasePath, config.server.bindAddress, config.server.port, config.server.maxConnections, 0, config.tls.certificateReference, config.runtime.queryTimeoutMs, config.runtime.checkpointWalBytes, config.runtime.bufferPoolBytes, config.runtime.temporaryMemoryBytes, config.server.maxStatementBytes, config.server.maxFrameBytes, config.server.maxResultRows, config.server.idleTimeoutMs)))
  end if
  if mode == "authenticated" then
    ...
    return serverResult(try(server.serveSecureAddressWithOperationalLimits(databasePath, config.server.bindAddress, config.server.port, config.server.maxConnections, 0, config.runtime.queryTimeoutMs, config.runtime.checkpointWalBytes, config.runtime.bufferPoolBytes, config.runtime.temporaryMemoryBytes, config.server.maxStatementBytes, config.server.maxFrameBytes, config.server.maxResultRows, config.server.idleTimeoutMs)))
  end if
  ...
  if mode == "standby" then
    ...
    return serverResult(try(server.serveStandbyConcurrentWithOperationalLimits(databasePath, config.server.port, config.server.maxConnections, 0, config.runtime.queryTimeoutMs, config.runtime.checkpointWalBytes, config.runtime.bufferPoolBytes, config.runtime.temporaryMemoryBytes, config.server.maxStatementBytes, config.server.maxFrameBytes, config.server.maxResultRows, config.server.idleTimeoutMs)))
  end if
